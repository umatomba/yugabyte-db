...
public class V221__Provider_persist_details extends BaseJdbcMigration {
  ...
  public void migrate(Connection connection) throws Exception {
    ...
    String selectStmt = "SELECT uuid, pgp_sym_decrypt(config, 'provider::config') as config, code FROM provider";
    ...
    while(providers.next()) {
      ...
      Map<String, String> config = (HashMap<String, String>) mapper.readValue(providers.getString("config"), HashMap.class);
      ...
      String updateStmt = String.format("UPDATE provider SET details = '%s' WHERE uuid = '%s'",  detailsSerialized, providerUUID);
